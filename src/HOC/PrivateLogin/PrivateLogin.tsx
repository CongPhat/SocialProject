...
interface RootState {
  LoginReducer: { privateLogin: boolean }
}
...
function PrivateLogin(Component: React.ComponentType<any | string>) {
  return withRouter(({ history }: RouteComponentProps) => {
    ...
    const login = useSelector((state: RootState) => state.LoginReducer.privateLogin)
    ...
    return <>{statusLogin && <Component privateLogin={login} />}</>
  })
}
